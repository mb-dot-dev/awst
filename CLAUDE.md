...
- `src/awst/screens/` holds one Textual `Screen` per page (`home.py`, `stacks.py`, `buckets.py`, `functions.py`, `queues.py`, `profiles.py` for the profile picker, `regions.py` for the region picker, `sso_login.py` for the SSO login modal, `delete_objects.py` for the delete progress modal shared by empty-bucket and object/prefix deletes, `objects.py` for the read-only S3 object browser (one screen per prefix level)), with list screens subclassing `ResourceListScreen` (`resource_list.py`), and pure presentation helpers (`formatting.py`). Screens load data with thread workers (`@work(thread=True, exclusive=True, exit_on_error=False)`) and handle results in `on_worker_state_changed`.
